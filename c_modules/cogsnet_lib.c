...
static PyObject *method_cogsnet(PyObject* self, PyObject* args) {
    ...
    // Parse arguments
    if (!PyArg_ParseTuple(args, "sifffiss", &forgettingType, &snapshotInterval, &mu, &theta, &lambda_, &units, &pathEvents, &delimiter)) {
        return NULL;
    }

    // Run cogsnet
    struct Cogsnet network = cogsnet(forgettingType, snapshotInterval, mu, theta, lambda_, units, pathEvents, delimiter);
    ...
    if(network.exitStatus == 0) {
        ...
        PyObject *exception = PyErr_Format(CogsnetException, "%s", network.errorMsg);
        return NULL;
    }
